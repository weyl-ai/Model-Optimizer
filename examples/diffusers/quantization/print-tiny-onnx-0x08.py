...
def make_transformer_block():
    ...
    input_tensor = helper.make_tensor_value_info("input", TensorProto.BFLOAT16, [1, 64, hidden])
    output_tensor = helper.make_tensor_value_info("output", TensorProto.BFLOAT16, [1, 64, hidden])
    ...
    # Weights
    qkv_w, qkv_s, qkv_g = make_fp4_weight("qkv", hidden * 3, hidden)
    out_w, out_s, out_g = make_fp4_weight("out", hidden, hidden)
    mlp_up_w, mlp_up_s, mlp_up_g = make_fp4_weight("mlp_up", mlp_hidden, hidden)
    mlp_down_w, mlp_down_s, mlp_down_g = make_fp4_weight("mlp_down", hidden, mlp_hidden)

    ln1_scale, ln1_bias = make_ln_params("ln1", hidden)
    ln2_scale, ln2_bias = make_ln_params("ln2", hidden)

    # Shape tensors
    shape_attn = helper.make_tensor("shape_attn", TensorProto.INT64, [4], [1, 64, heads, head_dim])
    shape_flat = helper.make_tensor("shape_flat", TensorProto.INT64, [3], [1, 64, hidden])
    shape_bmm = helper.make_tensor("shape_bmm", TensorProto.INT64, [3], [heads, 64, head_dim])
    shape_scores = helper.make_tensor("shape_scores", TensorProto.INT64, [3], [heads, 64, 64])
    shape_4d_scores = helper.make_tensor(
        "shape_4d_scores", TensorProto.INT64, [4], [1, heads, 64, 64]
    )
    shape_4d_out = helper.make_tensor(
        "shape_4d_out", TensorProto.INT64, [4], [1, heads, 64, head_dim]
    )

    scale_val = helper.make_tensor("scale_val", TensorProto.BFLOAT16, [], [1.0 / np.sqrt(head_dim)])
